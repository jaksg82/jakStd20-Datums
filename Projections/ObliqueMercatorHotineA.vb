...
Public Class ObliqueMercatorHotineA
    ...
    Overrides ReadOnly Property Type As Method
        Get
            Return Method.ObliqueMercatorHotineA
        End Get
    End Property
    ...
    Public Overrides Function FromGeographic(point As Point3D) As Point3D
        ...
        Try
            ...
            'Projection costants
            Dim B, A, TOr, D, F, H, G, GammaO, LonO As Double
            ...
            u2 = A * Math.Atan((S * Math.Cos(GammaO) + V1 * Math.Sin(GammaO)) / Math.Cos(B * (point.X - LonO))) / B
    ...
    Public Overrides Function ToGeographic(point As Point3D) As Point3D
        ...
        Try
            ...
            u1 = (point.Y - iNorthO) * Math.Cos(iAngle) + (point.X - iEastO) * Math.Sin(iAngle)
            Qi = Math.E ^ -(B * v1 / A)
            Si = (Qi - 1 / Qi) / 2
            Ti = (Qi + 1 / Qi) / 2
            Vi = Math.Sin(B * u1 / A)
    ...
    Public Overrides Function GetParams() As List(Of ParamNameValue)
        ...
        tmpList.Add(New ParamNameValue("False Easting", FalseEasting, ParamType.EastNorth, False))
        tmpList.Add(New ParamNameValue("False Northing", FalseNorthing, ParamType.EastNorth, True))
