...
Public Class StereographicPolarB
    ...
    Overrides ReadOnly Property Type As Method
        Get
            Return Method.StereographicPolarB
        End Get
    End Property
    ...
    Public Overrides Function FromGeographic(point As Point3D) As Point3D
        ...
        Try
            ...
            'From LL to EN
            Dim dE, dN, p, t, tf, mf, ko As Double
            If iLat1 < 0 Then
                'South pole case
                ...
                p = 2 * axis * ko * t / (((1 + ecc) ^ (1 + ecc) * (1 - ecc) ^ (1 - ecc)) ^ 0.5)
    ...
    Public Overrides Function ToGeographic(point As Point3D) As Point3D
        ...
        Try
            ...
            'From EN to LL
            Dim X, p, t, E1, E2, E3, E4, tf, mf, ko As Double
            If iLat1 < 0 Then
                'South pole case
                tf = Math.Tan(Math.PI / 4 + iLat1 / 2) / (((1 + ecc * Math.Sin(iLat1)) / (1 - ecc * Math.Sin(iLat1))) ^ (ecc / 2))
                mf = Math.Cos(iLat1) / (1 - ecc ^ 2 * Math.Sin(iLat1) ^ 2) ^ 0.5
                ...
                t = (p * (((1 + ecc) ^ (1 + ecc) * (1 - ecc) ^ (1 - ecc)) ^ 0.5)) / (2 * axis * ko)
