...
Public Class LambertConicWest
    ...
    Overrides ReadOnly Property Type As Method
        Get
            Return Method.LambertConicConformalWest
        End Get
    End Property
    ...
    Public Overrides Function ToGeographic(point As Point3D) As Point3D
        ...
        Try
            ...
            Ff = mf / (n * tf ^ n)
            Rf = a * Ff * tf ^ n * iK0
            Ri = Math.Abs(((iEastO - point.X) ^ 2 + (Rf - (point.Y - iNorthO)) ^ 2) ^ 0.5)
            If n < 0 Then Ri = -Ri
            Ti = (Ri / (a * iK0 * Ff)) ^ (1 / n)
            ...
            Next
            thetai = Math.Atan((iEastO - point.X) / (Rf - (point.Y - iNorthO)))
