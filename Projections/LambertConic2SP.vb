...
Public Class LambertConic2SP
    ...
    Overrides ReadOnly Property Type As Method
        Get
            Return Method.LambertConicConformal2SP
        End Get
    End Property
    ...
    Public Overrides Function FromGeographic(point As Point3D) As Point3D
        ...
        Try
            ...
            t0 = Math.Tan(Math.PI / 4 - point.Y / 2) / ((1 - ecc * Math.Sin(point.Y)) / (1 + ecc * Math.Sin(point.Y))) ^ (ecc / 2)
            t1 = Math.Tan(Math.PI / 4 - iLat1 / 2) / ((1 - ecc * Math.Sin(iLat1)) / (1 + ecc * Math.Sin(iLat1))) ^ (ecc / 2)
            t2 = Math.Tan(Math.PI / 4 - iLat2 / 2) / ((1 - ecc * Math.Sin(iLat2)) / (1 + ecc * Math.Sin(iLat2))) ^ (ecc / 2)
            tf = Math.Tan(Math.PI / 4 - iLatO / 2) / ((1 - ecc * Math.Sin(iLatO)) / (1 + ecc * Math.Sin(iLatO))) ^ (ecc / 2)
            ...
            TmpResult.Y = iNorthO + Rf - r * Math.Cos(theta)
            TmpResult.Z = point.Z
            Return TmpResult
    ...
    Public Overrides Function ToGeographic(point As Point3D) As Point3D
        ...
        Dim Rf, m1, m2, t0, t1, t2, tf, n, Ri, thetainv, tmplat As Double

        Try
            'Ellipsoid costants
            f = BaseEllipsoid.Flattening
