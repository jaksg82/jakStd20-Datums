...
Public Class MercatorC
    Inherits Projections

    Private iLatO, iLat1, iLonO, iK0, iEastO, iNorthO As Double

    Overrides ReadOnly Property Type As Method
        Get
            Return Method.MercatorVariantC
        End Get
    End Property
    ...
    Public Sub New(refEllipsoid As Ellipsoid, longName As String, compactName As String, latOfFalseOrigin As Double, lonOfOrigin As Double,
                   latOfStandardParallel As Double, falseEast As Double, falseNorth As Double)
        FullName = longName
        ShortName = compactName
        iLat1 = Math.Abs(latOfStandardParallel)
        iLonO = lonOfOrigin
    ...
    Public Overrides Function FromGeographic(point As Point3D) As Point3D
        ...
        Try
            ...
            M = a * iK0 * Math.Log(Math.Tan((Math.PI / 4) + (iLatO / 2)) * ((1 - (e * Math.Sin(iLatO))) / (1 + (e * Math.Sin(iLatO)))) ^ (e / 2))
            'From LL to EN
            TmpResult.X = iEastO + a * iK0 * (point.X - iLonO)
            TmpResult.Y = (iNorthO - M) + a * iK0 * Math.Log(Math.Tan(Math.PI / 4 + (point.Y / 2)) * ((1 - e * Math.Sin(point.Y)) / (1 + e * Math.Sin(point.Y))) ^ (e / 2))
            TmpResult.Z = point.Z
            Return TmpResult
    ...
    Public Overrides Function ToGeographic(point As Point3D) As Point3D
        Dim a, e As Double
        Dim X, t, A1, A2, A3, A4, M As Double
        Dim TmpResult As New Point3D
        ...
        Try
            ...
            A4 = (4279 * e ^ 8 / 161280)
            'From EN to LL
            t = Math.E ^ ((iNorthO - M - point.Y) / (a * iK0))
