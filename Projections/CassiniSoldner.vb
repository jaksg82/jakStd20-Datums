...
Public Class CassiniSoldner
    ...
    Public Overrides Function ToGeographic(point As Point3D) As Point3D
        ...
        Try
            ...
            'From EN to LL
            Dim D, T1, M1, Mo, u1, e1, lat1, V1, P1 As Double
            Mo = axis * (A1 * iLatO - A2 * Math.Sin(2 * iLatO) + A3 * Math.Sin(4 * iLatO) - A4 * Math.Sin(6 * iLatO))
            e1 = (1 - (1 - ecc ^ 2) ^ 0.5) / (1 + (1 - ecc ^ 2) ^ 0.5)
            M1 = Mo + (point.Y - iNorthO)
    ...
    Public Overrides Function GetParams() As List(Of ParamNameValue)
        ...
        tmpList.Add(New ParamNameValue("Latitude of Origin", LatitudeOfOrigin, ParamType.LatLong, True))
    ...
    Public Overrides Function ToP190Header() As List(Of String)
        ...
        H1800 = H1800 & " 008" & If(FullName.Length > 44, FullName.Substring(0, 44), FullName.PadRight(44, " "c))
        ret.Add(H1800)
